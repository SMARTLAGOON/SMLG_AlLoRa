...
# board is the Edge, firmware, radio, sf, freq, bandwidth, proceed. Every one taken as offered.
_ALL_DEFAULTS = ("", "", "", "", "", "", "", "")
...
def test_the_plan_is_shown_first_and_no_leaves_the_desk_alone(tmp_path, monkeypatch, capsys):
    _ports(monkeypatch, EDGE_PORT, HUB_PORT)
    wire = FakeWire()
    code, tail = _run(("", "", "", "", "", "", "", "n"), tmp_path / "fleet", wire)
...
def test_the_open_posture_asks_for_the_number_both_ends_carry(tmp_path, monkeypatch):
    ...
    # posture=1 (open), edge, session id 7, firmware, radio, sf, freq, bandwidth, proceed
...
def test_a_second_edge_is_registered_under_a_name_of_its_own(tmp_path, monkeypatch):
    ...
    # posture, 2 edges, edge 1, edge 2, firmware, radio, sf, freq, bandwidth, proceed
    code, tail = _run(("", "2", "", "", "", "", "", "", "", ""), tmp_path / "fleet", wire)
...
def test_the_chosen_firmware_is_the_one_written(tmp_path, monkeypatch, capsys):
    image = tmp_path / "AlLoRa-t3s3-sx127x-firmware.bin"
    image.write_bytes(b"\x00")
    monkeypatch.setattr("tools.allora_provision.setup.firmware_images",
                        lambda *a, **k: [str(image)])
    _ports(monkeypatch, EDGE_PORT, HUB_PORT)
    wire = FakeWire()
    # posture, edge, firmware=1 (the image, not "keep"), radio, sf, freq, bandwidth, proceed
