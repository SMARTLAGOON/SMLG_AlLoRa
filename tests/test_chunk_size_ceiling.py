...
The PHY ceiling is 255 bytes at every SF and bandwidth for v3, and v2 keeps the SF-keyed
table it shipped with. What the ceiling itself should be lives in test_payload_ceiling.py;
this file is about the overhead subtracted from it.
"""
import json
...
def test_the_overhead_is_all_that_separates_v2_from_v3_at_sf12(tmp_path):
    # v3 reaches the PHY ceiling at SF12 like it does anywhere else, so the gap here is the
    # header gap and nothing else: 255 - 6 against 255 - 12. v2 stays on its frozen table,
    # which is why its number is 18 rather than 243.
    assert _node(tmp_path, "v3.json", chunk_size=255, sf=12).chunk_size == 249
