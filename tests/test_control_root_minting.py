...
def test_a_minted_artifact_is_accepted_by_the_gate_it_was_minted_for(tmp_path):
    ...
    artifact = root.mint(RF_CONFIG, TARGET_DEVICE_ID, 1, RF_PAYLOAD)
...
def test_an_artifact_from_another_root_is_refused(tmp_path):
    ...
    gate.consume(_artifact(tmp_path, stranger.mint(RF_CONFIG, TARGET_DEVICE_ID, 1, RF_PAYLOAD)),
...
def test_an_artifact_minted_for_another_node_is_refused(tmp_path):
    ...
    gate.consume(_artifact(tmp_path, root.mint(RF_CONFIG, OTHER_DEVICE_ID, 1, RF_PAYLOAD)),
                 Reception(source="hub"))
...
def test_a_generated_root_can_command_a_node_provisioned_with_its_public_half(tmp_path):
    ...
    gate.consume(_artifact(tmp_path, root.mint(RESET, TARGET_DEVICE_ID, 1)),
                 Reception(source="hub"))
...
def test_minting_refuses_arguments_a_node_could_never_accept():
    ...
    root = Control_Root(CONTROL_ROOT_PRIV)
    with pytest.raises(ValueError):
        root.mint(RF_CONFIG, b"\x00\x01\x02\x03", 1, RF_PAYLOAD)  # not a 32-byte target
    with pytest.raises(ValueError):
        root.mint(RF_CONFIG, TARGET_DEVICE_ID, 0, RF_PAYLOAD)     # below the first counter
    with pytest.raises(ValueError):
        root.mint(0x1FF, TARGET_DEVICE_ID, 1, RF_PAYLOAD)         # not a single byte


def test_minting_will_not_guess_a_counter():
    # A default would mint a second artifact carrying the number the first already used, and
    # every target refuses that as a replay. The failure is silent and looks like a radio
    # problem, so the caller has to say which number it is issuing.
    root = Control_Root(CONTROL_ROOT_PRIV)
    with pytest.raises(TypeError):
        root.mint(RF_CONFIG, TARGET_DEVICE_ID)
...
def test_a_minted_sequence_keeps_working_as_the_counter_advances(tmp_path):
    ...
    for counter in (1, 2, 3):
        gate.consume(
            _artifact(tmp_path, root.mint(RF_CONFIG, TARGET_DEVICE_ID, counter, RF_PAYLOAD),
                      name="c{}.bin".format(counter)),
            Reception(source="hub"))
